...
//ラジオボタンクリック(納期確定済み)
$(function(){
	$(document).on("click","#btn_state2",function() {
...
//ラジオボタンクリック(出庫済み)
$(function(){
	$(document).on("click","#btn_state3",function() {
	$('#selected_state').empty();
	$('<label class="btn btn-default" id="btn_state2"><input type="radio" name="radio_state_check2" autocomplete="off"> 発注済み</label>').appendTo("#selected_state");
	$('<label class="btn btn-success active" id="btn_state3"><input type="radio" name="radio_state_check3" autocomplete="off" checked> 入庫済み</label>').appendTo("#selected_state");
	$('input[name="deadline_checkbox_name"]').prop("checked", false);
...
$('#update_data').on('click', function() {
	...
	if (length_check2 == 1 && year_check2 == 1 && bar1_check2 == 1 && month_check2 == 1 && bar2_check2 == 1 && date_check2 ==1 && space_check2 == 1 && hour_check2 == 1 && colon_check2 == 1 && minute_check2 == 1 && millisec_check2 == 1 && lastday_check2 == 1){
		...
		if (radio_name == "radio_state_check2"){
			deadline_ok = 2;
		}
		else if(radio_name == "radio_state_check3"){
			deadline_ok = 3;
		}
	}
	else{
	...
	//変更前と変更後のデータ準備
	var this_in_data_sql = alasql('SELECT num FROM trans WHERE id = ?', [id])[0];
		//変更前データ数(変更前が出庫済みに限る：出庫済み以外から出庫済みに変更した場合、出庫数が純増のため)
		if(state_check == 3){
			var this_in_data = this_in_data_sql["num"]; 
		}
		else {
			this_in_data = 0;
		}
	var input_data = num; //入力中のデータ
	var warehouse_stock_latest = warehouse_stock + this_in_data - input_data;
	if (state_check == 3){  //変更前の状態が出庫済みか判別：入庫数の減少により在庫数がマイナスになることを防ぐ
		if (input_data - this_in_data < 0){ //入力数を減らしたときのチェック
			//全体数のチェック
			//入庫数読み込み
			var in_13_sql = alasql('SELECT SUM(num) FROM trans WHERE stock = ? AND purpose = 1 AND state = 3', [ bread_rows.trans.stock ])[0];
			var in_13 = in_13_sql["SUM(num)"]; //入庫済み
			...
			var out_28 = out_28_sql["SUM(num)"]; //棚卸(不足)数
			//変更前データ数
			var this_data_sql = alasql('SELECT num FROM trans WHERE id = ?', [id])[0];
			var this_in_data = this_data_sql["num"]; 

			var all_out_total = in_13 - out_26 + out_27 - out_28 - in_19 - this_in_data + input_data;
			if (all_out_total < 0){  //全体で在庫数がマイナスにならないことをチェック
				$("#too_much").empty();
				var too_much_text = '(データ変更によりこの商品の倉庫在庫数が ' + all_out_total + ' となります。先に出庫・返品・棚卸調整データを修正してください。)';
				...
				alasql("UPDATE trans SET purpose = 1, state = 2, date = '" + date + "', deadline = '" + deadline + "', num = " + num + ", shop = '" + shop + "' WHERE id = " + id);
				window.location.assign(last_url);
				}
				//全条件クリアでデータ更新 (出庫済み)
				if (date_ok == 1 && shop_ok == 1 && num_ok == 1 && deadline_ok == 3){
				alasql("UPDATE trans SET purpose = 1, state = 3, date = '" + date + "', deadline = '" + deadline + "', num = " + num + ", shop = '" + shop + "' WHERE id = " + id);
				window.location.assign(last_url);
				}
			}
		}
		else{ //入力数増加のため、入庫済みを発注済みに戻す場合チェック
			$("#too_much").empty();
			//全条件クリアでデータ更新 (発注済み)
			if (date_ok == 1 && shop_ok == 1 && num_ok == 1 && deadline_ok == 2){
				if( warehouse_stock_latest < 0){ //入力後に在庫数がマイナスとなる場合
					$("#too_much").empty();
					var too_much_text = '(データ更新によりこの商品の倉庫在庫数が ' + warehouse_stock_latest + ' となります。先に出庫・返品・棚卸調整データを修正してください。)';
					$("#too_much").append(too_much_text);
					$("#selected_num1").css("color","red");
					$("#order-form_number_span").css("color","red");
					$("#order-form_number_span").animate({opacity: 0.4},50);
			...
			//全条件クリアでデータ更新 (入庫済み)：入庫数増加なのでフリーパス
			else if (date_ok == 1 && shop_ok == 1 && num_ok == 1 && deadline_ok == 3){
				alasql("UPDATE trans SET purpose = 1, state = 3, date = '" + date + "', deadline = '" + deadline + "', num = " + num + ", shop = '" + shop + "' WHERE id = " + id);
				window.location.assign(last_url);
			}
		}
	}
	else{ //変更前が入庫済みデータでないので変更可
		//全条件クリアでデータ更新 (発注確定済み)
		if (date_ok == 1 && shop_ok == 1 && num_ok == 1 && deadline_ok == 2){
		alasql("UPDATE trans SET purpose = 1, state = 2, date = '" + date + "', deadline = '" + deadline + "', num = " + num + ", shop = '" + shop + "' WHERE id = " + id);
		window.location.assign(last_url);
